...
class Species(AtomCollection):
    ...
    def optimise(
        ...
        calc.run(input_filename=input_filename, temp_dir=temp_dir)

        method_name = "" if method is None else method.name
        if optimized_filename is not None:
            self.print_xyz_file(
                filename=optimized_filename, append=False
            )
    ...
    def single_point(
        ...
        input_filename: str = "temp_mol.xyz",
        file_identifier: str = ""
    ) -> None:
        """
        ...
        sp.run(temp_dir=temp_dir, input_filename=input_filename, file_identifier=file_identifier)
        return None
