...
class XTB(autode.wrappers.methods.ExternalMethodOEG):
    ...
    @staticmethod
    def output_filename_for(calc: "CalculationExecutor", file_identifier=""):
        return f"{calc.name}_{file_identifier}.out"
    ...
    def execute(self, calc: "CalculationExecutor", input_filename=None):
        ...
        @work_in_tmp_dir(
            filenames_to_copy=[input_filename],
        ...
        def execute_xtb(input_filename=None):
            # logger.info(f'Running XTB with: {" ".join(flags)}')
            run_external(
                params=[calc.method.path, input_filename] + flags,
    ...
    def _energy_from(self, calc: "CalculationExecutor", file_identifier="") -> PotentialEnergy:
    ...
    def gradient_from(self, calc: "CalculationExecutor", file_identifier=""):
        raw = []
        ...
        return Gradient(raw, units="Ha a0^-1")
