...
class CalculationExecutor:
    ...
    def run(self, temp_dir=None, input_filename=None, file_identifier="") -> None:
        ...
        if self.method.uses_external_io:
            # self.generate_input() #todo: this create .autode_calculations and test_sp_xtb.xyz
            self.output.filename = self.method.output_filename_for(self, file_identifier=file_identifier)
            self._execute_external(input_filename=input_filename)
            self.set_properties(file_identifier=file_identifier)
            self.clean_up()
    ...
    def _execute_external(self, input_filename=None) -> None:
        """
        ...
        self.output.clear()
        self.method.execute(self, input_filename=input_filename)

        return None

    @requires_output_to_exist
    def set_properties(self, file_identifier="") -> None:
        ...
        # todo: changed here GradientKeywords
        if not isinstance(keywords, kws.OptKeywords):
            self.molecule.energy = self.method.energy_from(self, file_identifier=file_identifier)
            self.molecule.gradient = self.method.gradient_from(self, file_identifier=file_identifier)
