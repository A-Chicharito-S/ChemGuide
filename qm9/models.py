...
class DistributionProperty:
    def __init__(self, dataloader, properties, num_bins=1000, normalizer=None, data_num_atoms=None, data_prop=None):
        self.num_bins = num_bins
        self.distributions = {}
        self.properties = properties
        for prop in properties:
            self.distributions[prop] = {}
            if data_num_atoms is not None and data_prop is not None:
                print('calling DistributionProperty with specified `num_atoms` & `prop`')
                self._create_prob_dist(data_num_atoms,
                                       data_prop[prop],
                                       self.distributions[prop])
            else:
